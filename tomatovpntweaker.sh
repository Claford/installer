...
then
	...
	echo ""
	read -p "Do you want to remove the TCP Tweaker settings? [y / n]:" -e -in answer0
	if [["$ answer0" = 's']]; then
		grep -v "^ # PH56
...
sysctl -p /etc/sysctl.conf> / dev / null
		echo ""
		echo "TCP Tweaker's network settings have been removed successfully."
		echo ""
	exit
	else 
		echo ""
	...
	echo ""
	read -p "Continue with the installation? [y / n]:" -e -in answer
	if [["$ answer" = 's']]; then
	echo ""
...
sysctl -p /etc/sysctl.conf
		echo ""
		echo "TCP Tweaker's network settings have been added successfully."
		echo ""
	else
		echo ""
